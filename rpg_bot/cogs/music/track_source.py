...
class TrackSource(discord.PCMVolumeTransformer):
    ...
    @classmethod
    async def from_url(cls, url, *, loop=None, stream=False, loop_stream=True):
        loop = loop or asyncio.get_event_loop()
        data = await cls.fetch_data(url, loop=loop, download=not stream)

        ffmpeg_configs = cls.ffmpeg_options

        if loop_stream:
            ffmpeg_configs = cls.ffmpeg_options_loop

        filename = data['url'] if stream else cls.ytdl.prepare_filename(data)
        return cls(discord.FFmpegPCMAudio(filename, **ffmpeg_configs),
                   data=data)
